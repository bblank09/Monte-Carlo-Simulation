...
from backend.app.engine.results import compute_var_es, percentile_table
...
def test_percentile_table_and_var_es():
    ...
    assert list(table["ending_balance"]) == [10, 25, 50, 75, 90]
    assert table["ending_balance"][10] < table["ending_balance"][90]
